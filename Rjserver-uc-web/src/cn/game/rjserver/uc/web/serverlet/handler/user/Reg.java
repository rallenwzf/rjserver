 ...
 * 
 */
@Cmd(action = 2, desc = "手动创建用户，返回用戶信息")
public class Reg implements ActionCommand {
	...
	public String execute(HttpServletRequest request) throws Exception {
		...
		defaultUser.setUserpwd(password);
		defaultUser.setUname("p_" + useraccount);
		//
		...
		TucUserinfo tuserinfo = UcProviderManager.getInstance().getUserProvider().createUser(defaultUser, defaultUserinfo, false);
		if (tuserinfo != null) {
