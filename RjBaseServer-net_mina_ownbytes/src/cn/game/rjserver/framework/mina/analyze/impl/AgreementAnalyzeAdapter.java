...
import cn.game.rjserver.framework.protocol.analyze.AgreementAnalyze;
 ...
 */
public class AgreementAnalyzeAdapter extends AgreementAnalyzeAbsimpl {
	...
	public void encode(Agreement ag, Object bufObj, GameIoSession session) {
		...
		try {
			...
			ag.setServer(true);// 服务器协议，发送给客户端
			Header h = ag.getHeader();
			if (h == null) {
				h = ServerToCommand.getInstance().getDefaultHeader();
				ag.setHeader(h);
			}
			if (session != null && session.getAttribute("IsManagerAG") != null) {
				//
			...
			// 信息头+消息体
			short s = (short) (h.getPackgeHeaderLength() + Constants.SERVERREQUEST_CONTENT_HEADER_LENGTH + ag.getDataLength(true));
			h.setDataLength(s);
